...
void read_row_striped_matrix (
   ...
   void       **storage,  /* OUT - Submatrix stored here 如果輸出是指標，這邊就要寫指標指標*/
   MPI_Datatype dtype,    /* IN - Matrix element type */
   int         *m,        /* OUT - Matrix rows 如果輸出是值，這邊就要寫指標*/
   ...
   MPI_Comm     comm)     /* IN - Communicator */
{
   ...
   MPI_Comm_size (comm, &p); /* 為了減少傳入值，所以這邊直接跟mpi要process跟id */
   MPI_Comm_rank (comm, &id);
   datum_size = get_size (dtype);

   /* Process p-1 opens file, reads size of matrix,
      and broadcasts matrix dimensions to other procs (ch6 ppt p16由最後一個process來讀file)*/

   if (id == (p-1)) {
      infileptr = fopen (s, "r");
      if (infileptr == NULL) *m = 0; /* 開檔失敗，matrix row(m)=0 */
      else {
         fread (m, sizeof(int), 1, infileptr);
         fread (n, sizeof(int), 1, infileptr);
      }      
...
/* 如果m大小是0，直接結束mpi程式(MPI_Abort) */
   ...
   */
   ...
   /* Process p-1 reads blocks of rows from file and
      ...
      (ch6 ppt p16由最後一個process來讀file)
      最後一個process的block一定是長的size，所以一定放得下前面幾個process的block*/
